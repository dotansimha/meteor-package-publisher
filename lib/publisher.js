    ...
    readFile(function(fileString) {
        ...
        var handleSectionWithPlatforms = function(arr, addCb) {
            ...
            _.forEach(arr, function(value) {
                returnString += addCb(value.name, JSON.stringify(value.platforms));
            });

            return returnString;
        };

        fileString = fileString.replace('DEPS_SECTION', handleSectionWithPlatforms(that._destination.use, function(name, platforms) {
            return "api.use('" + name + "', " + platforms + ");\n";
        }));
...
MeteorPublisher.prototype.publish = function() {
    var that = this;
    var deferred = Q.defer();

    var command = 'meteor publish ' + (this._destination.create ? ' --create' : '');
